...
const generateWithOptions = ({
        ...
        templates
          .filter(template => {
            if (template.platform) {
              return platforms.includes(template.platform)
            }
  ...
  const generateExampleWithName = exampleName => {
    const exampleReactNativeInitCommand = `react-native init ${exampleName} --version ${exampleReactNativeVersion}`
  ...
  return generateWithoutExample().then(() => {
    return generateExample ? generateExampleWithName(exampleName) : Promise.resolve()
  })
}
