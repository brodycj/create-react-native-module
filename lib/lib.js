...
const generateWithNormalizedOptions = ({
  ...
  const generateLibraryModule = () => {
    return fs.ensureDir('tmp-'+moduleName).then(() => {
      return Promise.all(templates.filter((template) => {
        ...
        return renderTemplateIfValid(fs, 'tmp-'+moduleName, template, templateArgs);
      }));
    }).then(() => fs.ensureDir(moduleName)
    ).then(() => fsExtra.copy('tmp-'+moduleName, moduleName));
  };
