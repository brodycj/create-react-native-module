...
// FUTURE TODO to be replaced by *real* execa import:
const { execSync } = require('child_process');

// External imports:

const fsExtra = require('fs-extra'); // default fs object

// Internal imports:

const normalizedOptions = require('./normalized-options');

// FUTURE TODO remaining modules in utils to be factored out:
const { npmAddScriptSync, exec } = require('./utils');
...
const execaFake = { commandSync: execSync };
...
const renderTemplateIfValid = (myfs, root, template, templateArgs) => {
  // avoid throwing an exception if no valid
  // template.name member exists here:
  ...
  return myfs.ensureDir(baseDir).then(() =>
    myfs.outputFile(filename, template.content(templateArgs))
  );
};
...
const generateWithOptions = ({
  ...
  myfs = fsExtra, // (this can be mocked out for testing purposes)
  execa = execaFake, // (this can be mocked out for testing purposes)
}) => {
  ...
  if (generateExample) {
    ...
    try {
      ...
      execa.commandSync(reactNativeVersionCommand, checkCliOptions);
    ...
    try {
      ...
      execa.commandSync(yarnVersionCommand, checkCliOptions);
  ...
  const generateWithoutExample = () => {
    return myfs.ensureDir(moduleName).then(() => {
      return Promise.all(templates.filter((template) => {
        ...
        return renderTemplateIfValid(myfs, moduleName, template, templateArgs);
      }));
    });
  };
  ...
  const generateExampleWithName =
    (exampleName) => {
      ...
      return Promise.resolve()
        .then(() => {
          // We use synchronous call here to be able to output the stdout to stdout
          // (Note that any exception would be propoerly handled since this is
          //  within a Promise.resolve().then() callback.)
          execa.commandSync(exampleReactNativeInitCommand, execOptions)
        })
        .then(() => {
        ...
        .then(() => {
          ...
          return new Promise((resolve, reject) => {
            ...
            try {
              execa.commandSync('yarn add file:../', addLinkLibraryOptions);
            } catch (e) {
              console.error('Yarn failure for example, aborting');
              throw (e);
            }
            execa.commandSync('react-native link', addLinkLibraryOptions);
