...
const vfs = require('vinyl-fs-mock');
...
const generateWithNormalizedOptions = ({
  ...
  const generateLibraryModule = () => {
    return fs.ensureDir(moduleNameTmp).then(() => {
      ...
      return fsExtra.ensureDir(moduleName);
    }).then(() => {
      // const fs = vfs;
      // const fs = vfs('/', {});
      const fs = vfs(moduleName, {});
      ...
      pump(
        gulp.src([`${moduleNameTmp}/**/*`]),
        // gulp.dest(moduleName)
        fs.dest(moduleName),
        // () => console.log(fs.directory),
        // () => fsExtra.outputFile(`${moduleName}/aaa`, JSON.stringify(fs.directory))
        // () => fsExtra.outputFile(`${moduleName}/aaa`, JSON.stringify(fs.directory[moduleName]))
        () => fsExtra.outputFile(`${moduleName}/aaa`, ' '.concat(fs.directory[moduleName]))
      )
      // return resolve();
    });
  };
