...
const generateWithNormalizedOptions = ({
  ...
  const execCommand = execa.command;
  ...
  const checkToolsForExampleApp = newPromisePipeline(
    ...
    ({ reactNativeVersionCommand, yarnVersionCommand, checkCliOptions, errorRemedyMessage }) => (
      startPromisePipeline(
        ...
        () => (
          // asynchronous system command call with very simple error handling
          Promise.resolve().then(
            thenCallWithArguments(execCommand)(reactNativeVersionCommand, checkCliOptions)
          ).catch((e) => {
        ...
        () => (
          // system command call with very simple error handling
          Promise.resolve().then(
            thenCallWithArguments(execCommand)(yarnVersionCommand, checkCliOptions)
          ).catch((e) => {
  ...
  const generateExampleApp = newPromisePipeline(
    thenStartPipelineStageWithProperties({
      ...
      startPromisePipeline([
        ...
        thenCallWithArguments(execCommand)(exampleReactNativeInitCommand, execOptions),
        thenStartPipelineStageWithProperties(
          {
            name: className,
            moduleName,
            ...
            ({ jsonPath }) => startPromisePipeline(
              ...
              () =>
                Promise.resolve().then(
                  ...
                  thenCallWithArguments(jsonfile.readFile)(jsonPath, { fs })
                ).catch(e => {
              ...
              (jsonOutput) =>
                Promise.resolve().then(
                  thenCallWithArguments(jsonfile.writeFile)(jsonPath, jsonOutput, { fs, spaces: 2 })
                )
            ),
            thenStartPipelineStageWithProperties(
              ...
              ({ addLinkLibraryOptions }) => startPromisePipeline(
                ...
                () =>
                  Promise.resolve().then(
                    thenCallWithArguments(execCommand)('yarn add file:../', addLinkLibraryOptions),
                  ).catch(e => {
                    console.error('Yarn failure for example, aborting');
                    throw (e);
                  }),
                thenCallWithArguments(execCommand)('react-native link', addLinkLibraryOptions),
              )
            ),
          )
        )
