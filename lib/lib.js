...
// default fs objects
const fsPromisesDefault = require('fs').promises;
const fsGracefulDefault = require('graceful-fs');
...
const renderTemplateIfValid = (fs, root, template, templateArgs) => {
  ...
  return fs.mkdir(baseDir, { recursive: true }).then(() =>
    fs.writeFile(filename, template.content(templateArgs))
  );
};
...
const generateWithNormalizedOptions = ({
  ...
  fsPromises = fsPromisesDefault, // (this can be mocked out by injection for testing purposes)
  fsGraceful = fsGracefulDefault, // (this can be mocked out by injection for testing purposes)
  execa = execaDefault, // (this can be mocked out _by injection_ for testing purposes)
}) => {
  // used by non-JSON fs calls
  const fs = fsPromises;
  ...
  const generateLibraryModule = () => {
    return fs.mkdir(moduleName, { recursive: true }).then(() => {
