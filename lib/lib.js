...
const {
  ...
  DEFAULT_LICENSE,
  DEFAULT_GENERATE_EXAMPLE,
  DEFAULT_EXAMPLE_NAME,
  DEFAULT_EXAMPLE_REACT_NATIVE_VERSION,
...
` : ``));
  ...
  const generateExampleApp = () => {
    const exampleReactNativeInitCommand =
        `npx react-native init ${exampleName} --template ${exampleReactNativeTemplate}@${exampleReactNativeVersion}`;
      ...
      .then(() => {
        ...
        return new Promise((resolve, reject) => {
          ...
          if (platforms.indexOf('ios') !== -1) {
            const iosExampleAppProjectPath = `${pathExampleApp}/ios`;
            ...
            try {
              log(`check for valid pod version in ${iosExampleAppProjectPath}`);
