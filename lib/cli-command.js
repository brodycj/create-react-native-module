...
${postCreateInstructions(createOptions)}`);
  ...
  options: [{
    ...
    description: 'Generate an example project (new architecture)',
  }, {
    command: '--example-name [exampleName]',
    description: 'Name for the example project (ignored when `--generate-example` is unset)',
    default: 'example',
  }, {
    command: '--example-react-native-template [exampleReactNativeTemplate]',
    description: 'React Native template for the generated example project (if you want to use typescript template, use "react-native-template-typescript") (ignored when `--generate-example` is unset)',
    default: 'react-native',
  }, {
    command: '--use-typescript',
    description: 'Generate TypeScript files for src and examples',
  }, {
    command: '--patch-unified-example',
    description: 'Patch unified package.json by library and example (beware of --example-react-native-template parameter) (ignored when `--generate-example` is unset)'
  }, {
    command: '--use-swift',
    description: 'Generate Swift files for src',
  }, {
    command: '--use-kotlin',
    description: 'Generate Kotlin files for src (also apply example due to lib works fine)',
  }]
};
