...
${postCreateInstructions(createOptions)}`);
  ...
  options: [{
    ...
    description: 'Generate an example project and add the library module to it with symlink by default, with overwrite of example metro.config.js to add workaround for Metro symlink issue - requires both react-native-cli and yarn to be installed globally',
  }, {
