...
test('CLI creates correct package artifacts on file system, with no options', async () => {
  ...
  const cmdOptions =
    `--generate-example --example-react-native-version react-native@0.59.10`;
  ...
  for (const path of files) {
    ...
    // TBD this is a quick trick to deal with rqndom UUIDs in project.pbxproj:
    const theContent =
      content
        .replace(/\t\t.*IntegrationTest/g, '\t\t... ... ...IntegrationTest');
